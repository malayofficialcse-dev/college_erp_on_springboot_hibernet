...
const api = axios.create({
  baseURL: 'http://localhost:8080/api', // Toggle USE_MOCKS below to point to a real Spring Boot server
  headers: {
...
// Toggle this to false when connecting to a real Spring Boot backend
  ...
  try {
    ...
    if (url.includes('/employees/search') || (url.endsWith('/employees') && method === 'get')) {
      ...
      if (params.department) {
        const depts = params.department.split(',');
